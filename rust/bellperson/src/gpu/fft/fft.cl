...
KERNEL void all_in_one(
    GLOBAL FIELD *x,      // Source buffer
...
{
// CUDA doesn't support local buffers ("shared memory" in CUDA lingo) as
// function arguments, ignore that argument and declare the external memory here
// instead.
#ifdef CUDA
...
/// Multiplies all of the elements by `field`
KERNEL void mul_vec(GLOBAL FIELD *elements, LOCAL FIELD *field) {
  const uint gid = GET_GLOBAL_ID();
  elements[gid] = FIELD_mul(elements[gid], field[0]);
}

/// distribute_powers
KERNEL void distribute_powers(GLOBAL FIELD *elements, LOCAL FIELD *field) {
  const uint gid = GET_GLOBAL_ID();
  FIELD tmp = FIELD_pow(field[0], gid);
  elements[gid] = FIELD_mul(elements[gid], tmp);
}

/// vector mul
KERNEL void mul_vector(GLOBAL FIELD *elements, GLOBAL FIELD *field) {
  const uint gid = GET_GLOBAL_ID();
  elements[gid] = FIELD_mul(elements[gid], field[gid]);
}

/// vector sub
KERNEL void sub_vector(GLOBAL FIELD *elements, GLOBAL FIELD *field) {
  const uint gid = GET_GLOBAL_ID();
  elements[gid] = FIELD_sub(elements[gid], field[gid]);
}
