...
const MEMORY_PADDING: f64 = 0.0f64; // Let 20% of GPU memory be free
            ...
            if n > 0 {
                    ...
                    .zip(results.iter_mut())
                {
                    ...
                    s.execute(move || {
                        ...
                        let mut size_chunk = kern.n;
                        let data_size = std::mem::size_of_val(&bases[0]);
                        ...
                        info!(
                            "************* bases len: {}, size {} *************",
                            size_chunk, data_size
                        );

                        for (bases, exps) in bases.chunks(size_chunk).zip(exps.chunks(size_chunk)) {
