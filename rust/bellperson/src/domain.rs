...
impl<E: Engine + gpu::GpuEngine> EvaluationDomain<E> {
    ...
    pub fn all_in_one_ori<'a>(
        ...
        // let result = std::sync::Arc::new(std::sync::RwLock::new(Ok(())));

        if let Some(ref mut kern) = kern {
            let geninv = a.geninv;
            let e = E::Fr::multiplicative_generator();
            let az = a.z(&e).invert().unwrap();
            let log_nn = a.exp;
            ...
            if kern
                .with(|k: &mut gpu::FFTKernel<E>| {
                    ...
                    // let inputs = &mut vec![&mut a.coeffs[..], &mut b.coeffs[..], &mut c.coeffs[..]];
                    // let omegas = &[a.omega, b.omega, c.omega];
                    // let log_ns = &[a.exp, b.exp, c.exp];

                    // let n = inputs.len();
                    let num_devices = k.kernels.len();
                    info!(
                        "=======================FFT all in one: {} devices================",
                        num_devices
                    );
                    ...
                    THREAD_POOL.scoped(|s| {
                        ...
                        let kern = k.kernels.iter_mut().next().unwrap();
                        // use rust_gpu_tools::Device;
                        // let device = Device::all();
                        // let mut kern = SingleFftKernel::<E>::create(&device[0], true).unwrap();

                        let mut inputs =
                            [[&mut a.coeffs[..], &mut b.coeffs[..], &mut c.coeffs[..]]];
                        let omegas = [a.omegainv];
                        let omega_as = [a.omega];
                        let minvs = [[a.minv, b.minv, c.minv]];
                        let d_powers = [[e]];
                        let d_powers_a = [[geninv]];
                        let log_ns = [log_nn];
                        let az = [[az]];

                        s.execute(move || {
                            for (
                                ((((((input, omega), omega_a), minv), d_power), d_powers_a), log_n),
                                az,
                            ) in inputs
                                .iter_mut()
                                .zip(omegas.iter())
                                ...
                                .zip(az.iter())
                            {
                                ...
                                if let Err(err) = kern.all_in_one(
                                    input, omega, omega_a, minv, d_power, d_powers_a, *log_n, az,
                                ) {
                ...
                .is_ok()
            {
                info!("all in one finished!");
            } else {
                panic!("GPU resource not found! Cannot run all in one FFT!1");
            }
        } else {
            panic!("GPU resource not found! Cannot run all in one FFT!2");
        }
        ...
        // EvaluationDomain::ifft_many(&mut [&mut a, &mut b, &mut c], &worker, kern);
        // EvaluationDomain::coset_fft_many(&mut [&mut a, &mut b, &mut c], &worker, kern);
    ...
    // let mut k = gpu::FFTKernel::<E>::create(true).expect("Cannot initialize kernel!");

    // if k.all_in_one(
    //     &mut [&mut a.coeffs, &mut b.coeffs, &mut c.coeffs],
    //     &a.omegainv,
    //     &a.omega,
    //     &vec![a.minv, b.minv, c.minv],
    //     &vec![e],
    //     &vec![geninv],
    //     log_nn,
    //     &vec![az],
